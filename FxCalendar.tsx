...
export default function FxCalendar({ 
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(() => {
      ...
      // ---- 5 MINUTES PRE-ALERT ----
      // Wide window: 270–330 seconds (4.5 to 5.5 minutes) to never miss the trigger
      if (soundEnabled) {
        const upcoming5MinEvents = currentDynamicEvents.filter(e => {
          ...
          return diffSeconds >= 270 && diffSeconds <= 330;
        });

        if (upcoming5MinEvents.length > 0) {
          const eventStamp = upcoming5MinEvents[0].date;
          if (!fiveMinAlerted.has(eventStamp)) {
            fiveMinAlerted.add(eventStamp);
            setAlertedTimestamps(prev => [...prev, eventStamp]);
            const titles = upcoming5MinEvents.map(ue => `[${ue.country}] ${ue.title}`).join(', ');
            executeFiveStrikeAlarm(`[5m] ${titles}`, soundProfile);
          }
        }
      }

      // ---- 30 MINUTES PRE-ALERT ----
      // Wide window: 1770–1830 seconds (29.5 to 30.5 minutes) to never miss the trigger
      if (soundEnabled30Min) {
        const upcoming30MinEvents = currentDynamicEvents.filter(e => {
          if (!shouldAlert(e)) return false;
          const diffSeconds = (new Date(e.date).getTime() - currTimeMs) / 1000;
          return diffSeconds >= 1770 && diffSeconds <= 1830;
        });

        if (upcoming30MinEvents.length > 0) {
          const eventStamp = upcoming30MinEvents[0].date;
          if (!thirtyMinAlerted.has(eventStamp)) {
            thirtyMinAlerted.add(eventStamp);
            setAlertedTimestamps30Min(prev => [...prev, eventStamp]);
            const titles = upcoming30MinEvents.map(ue => `[${ue.country}] ${ue.title}`).join(', ');
            executeFiveStrikeAlarm(`[30m] ${titles}`, soundProfile);
          }
        }
      }
    }, 5000);
